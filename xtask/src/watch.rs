...
impl Watch {
  ...
  pub fn run(mut self, mut command: Command) -> Result<()> {
    ...
    let mut watcher =
      RecommendedWatcher::new(tx, Config::default()).context("could not initialize watcher")?;
    ...
    for event in rx {
      match event {
        Ok(Event { paths, kind, .. }) if command_start.elapsed() >= self.debounce => {
          match kind {
            event::EventKind::Create(_) | event::EventKind::Modify(_) => {
              for path in paths {
                ...
                log::trace!("Detected changes at {} | {:?}", path.display(), kind);
            ...
            event => log::trace!("Ignoring changes in {:?}", event),
          }
        }
