...
//!   - Re-run a given command when changes are detected (using [xtask-watch](https://github.com/rustminded/xtask-watch)).
//! * [`DevServer`](crate::dev_server::DevServer) - Serve your project at a
//!   given IP address.
//!
...
//!   ```
//!
//! * Serve an optimized web app dist on `127.0.0.1:8000` and watch for changes
...
//!
//! This example also demonstrates the use of the `run-example` feature that
...
//! ```
//!
//! This command will run the code in `examples/run_example` using the
//! development server.
//!
//! # Features
//!
...
//! * `run-example`: a helper to run examples from `examples/` directory using a
//!   development server.
