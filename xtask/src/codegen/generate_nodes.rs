...
pub fn generate_nodes(ast: &AstSrc) -> Result<String> {
  let (node_defs, node_boilerplate_impls): (Vec<_>, Vec<_>) = ast
    ...
    .map(|node| {
      ...
      let node_kind = format_ident!("{}", node.name.as_str().to_shouty_snake_case());

      let methods = node.fields.iter().enumerate().map(|(slot_index, field)| {
        match field {
  ...
  let (union_defs, union_boilerplate_impls): (Vec<_>, Vec<_>) = ast
    ...
    .map(|union| {
      ...
      let as_method_for_variants_for_union: Vec<_> = union
        ...
        .map(|variant| {
          let variant_name = format_ident!("{}", variant);
          let fn_name = format_ident!("as_{}", variant.to_snake_case());
          quote! {
      ...
      //
      // Inside an enum, we can have variants that point to a "flat" type or to
      // another enum; we want to divide these variants as we will generate a
      // different code based on these requirements
      ...
      let vv: Vec<_> = variant_of_variants
        ...
        .map(|(i, en)| {
          ...
          let variable_name = format_ident!("{}", en.as_str().to_snake_case());
          (
            // cast() code
  ...
  let unknowns = ast.unknowns.iter().map(|unknown| {
    ...
    let kind = format_ident!("{}", unknown.to_shouty_snake_case());
  ...
  let lists = ast.lists().map(|(name, list)| {
    ...
    let list_kind = format_ident!("{}", name.to_shouty_snake_case());
  ...
  let syntax_element_children = shared_quotes.syntax_element_children();
  let syntax_list = shared_quotes.syntax_list();
  let syntax_token = shared_quotes.syntax_token();
  let language = shared_quotes.language();
...
pub(crate) fn token_kind_to_code(name: &str) -> proc_macro2::TokenStream {
  let kind_variant_name = name.to_shouty_snake_case();

  if KINDS.literals.contains(&kind_variant_name.as_str())
    || KINDS.tokens.contains(&kind_variant_name.as_str())
  {
