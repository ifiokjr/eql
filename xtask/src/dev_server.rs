...
/// use xtask_wasm::clap;
/// use xtask_wasm::default_dist_dir;
///
/// #[derive(clap::Parser)]
/// enum Opt {
...
impl DevServer {
  ...
  pub fn start(self, served_path: impl AsRef<Path>) -> Result<()> {
    let watch_process = if let Some(command) = self.command {
      // NOTE: the path needs to exists in order to be excluded because it is
      // canonicalize
      let _ = std::fs::create_dir_all(&served_path);
      let watch = self.watch.exclude_path(&served_path);
      let handle = std::thread::spawn(|| {
        match watch.run(command) {
