...
use crate::default_build_command;
use crate::metadata;
...
/// `my-project` crate. It will run the
/// [`default_build_command`](crate::default_build_command) at the workspace
/// root, copy the content of the `project/static` directory, generate JS
/// bindings and output two files: `project.js` and `project.wasm` into the dist
/// directory.
#[non_exhaustive]
#[derive(Debug, Parser)]
#[clap(
...
fn sass(
  ...
  for entry in walker {
    let entry =
      entry.with_context(|| format!("cannot walk into directory `{}`", &static_dir.display()))?;
    ...
    if !source.is_file() {
      ...
      if !should_ignore(source) {
        ...
        let css =
          sass_rs::compile_file(source, options.clone()).expect("could not convert SASS/ file");
        fs::write(&dest, css)
          .with_context(|| format!("could not write CSS to file `{}`", dest.display()))?;
      }
    } else {
      fs::copy(source, &dest)
        .with_context(|| format!("cannot move `{}` to `{}`", source.display(), dest.display()))?;
    }
  }
